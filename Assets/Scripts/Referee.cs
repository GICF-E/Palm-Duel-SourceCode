...
public class Referee : MonoBehaviour
{
    ...
    [Tooltip("我方玩家")] public Player player1;
    [Tooltip("对方玩家")] public Player player2;
    ...
    [HideInInspector][Tooltip("是否正在处理卡牌")] public bool isProcessingCard = false;
    ...
    private IEnumerator ClapRoutine()
    {
        ...
        while (player1.stats.hp > 0 && player2.stats.hp > 0)
        {
            ...
            // 等待双方选择卡牌并且动画播放完毕
            yield return new WaitUntil(() => player1.selectCard != null && player2.selectCard != null && animator.GetCurrentAnimatorStateInfo(0).normalizedTime >= 1);
            ...
            if (player1.stats.freezeTime == 0)
            {
                ...
                player1.selectionIndicator.color = Color.grey;
            }
            if (player2.stats.freezeTime == 0)
            {
                player2.selectCard = null;
                player2.selectionIndicator.color = Color.grey;
            }
    ...
    // 计算总伤害（反伤将被直接执行）
    private float CalculateTotalDamage(Player attacker, Player defender, Move attackerMove, Move defenderMove)
    {
        // 检查是否被冷冻
        if (attacker.stats.freezeTime == 0)
        {
            ...
            foreach (var ability in attackerMove.abilityIncrease)
            {
                ...
                // 增加总和
                // 确保增加的不是点
                if (ability.key != "点" && ability.key != "总和") attacker.stats.abilityPoints["总和"] += ability.value;
            }
    ...
    // 检查玩家是否死亡
    private bool CheckForDeath(Player player)
    {
        // 如果玩家生命值小于等于0
    ...
    private IEnumerator HandleGameEnd(bool isWin)
    {
        ...
        // 计算自己要改变的经验值
        int skillChange = isWin ? Mathf.RoundToInt(UserData.GetRemotePlayerSkill() * 0.5f) : -Mathf.RoundToInt(UserData.GetPlayerSkill() * 0.5f);
        ...
        yield return StartCoroutine(VerifyUserData(isVerified =>
        {
            if (isVerified)
            {
                ...
                // 将新记录添加到头部
                UserData.historyRecords.Insert(0, (UserData.remotePlayerDisplayName, isWin ? $"+{Mathf.Abs(skillChange)}" : $"-{Mathf.Abs(skillChange)}"));

                // 更新经验值并提交
                UserData.UpdatePlayerSkill(UserData.GetPlayerSkill() + skillChange);
                UserData.UpdateRemotePlayerSkill(UserData.GetRemotePlayerSkill() - skillChange);
    ...
    public void BanPlayer(string playFabId, string reason, int? durationInHours)
    {
        ...
        PlayFabClientAPI.ExecuteCloudScript(
            ...
            {
                Debug.Log("成功调用云脚本");
                Debug.Log($"返回结果: {JsonUtility.ToJson(result.FunctionResult)}");
            },
            error =>
            {
    ...
    // 特殊卡牌处理
    public void SpecialPropertyHandling(Player attacker, Player defender, Move attackerMove, Move defenderMove)
    {
    ...
    // 特殊卡牌效果方法
    // 闪
    private void Card_Dodge(Player attacker)
    {
        // 标记已经出过闪
    ...
    // 湮灭
    private void Card_Annihilate(Player defender)
    {
        // 对手直接死亡，返回最大生命值作为伤害
    ...
    // 满血复活
    private void Card_FullBloodResurrection(Player attacker)
    {
        // 增加无敌时间
    ...
    // 点Chee
    private void Card_DotChee(Player defender, Move defenderMove)
    {
        // 判断对方是否出拳头
    ...
    // 眩晕电击
    private void Card_StunLightning(Player defender, Move defenderMove)
    {
        // 判断对方是否出闪
    ...
    // 核电站
    private void Card_NuclearPowerStation(Player attacker)
    {
        // 增加持续伤害
    ...
    // 流沙陷阱
    private void Card_QuicksandTrap(Player attacker)
    {
        // 增加反伤时间
    ...
    // 火药
    private void Card_Gunpowder(Player attacker)
    {
        // 设置下次伤害乘区
    ...
    // 鱼跃龙门
    private void Card_CarpLeap(Player attacker)
    {
        // 增加无敌时间
        attacker.stats.invulnerableTime += 5;
    }
    ...
    // 冷冻
    private void Card_Freeze(Player attacker, Player defender, Move attackerMove, Move defenderMove)
    {
        // 计算当前自己会收到的伤害
        // 计算基础伤害
