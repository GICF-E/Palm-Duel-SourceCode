...
public class PlayerAgent : Agent
{
    ...
    [Tooltip("跟踪当前暂停协程")] private Coroutine currentPauseCoroutine;
    [Tooltip("我方玩家统计信息")] public PlayerStats stats;
    [Tooltip("对方玩家统计信息")] public PlayerAgent opponentPlayer;
    [Tooltip("卡牌代码列表")] public List<CardForAgent> cards;
    [Tooltip("卡牌交互暂停事件")] public float cardInteractionPauseTime;
    [HideInInspector][Tooltip("当前选择的卡牌")] public CardForAgent selectCard;
    ...
    [Header("引用")]
    [Tooltip("Referee")] public RefereeForAgent referee;
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        {
            // 选择卡牌
            StartCoroutine(SelectCard(selectedCardIndex));
        }
        else
        {
    ...
    // 选择卡牌
    private IEnumerator SelectCard(int selectedCardIndex)
    {
        // 等待玩家完成动作
        yield return new WaitUntil(() => referee.player1.selectCard != null);
        // 在远程玩家的卡牌列表中查找选择的卡牌
        CardForAgent selectedCard = cards.Find(c => c.cardName == movesList.moves[selectedCardIndex].name);
        // 切换卡牌状态为选中状态
        selectedCard.ChangeCardState();
    }

    public IEnumerator PauseCardSelection()
    {
        // 如果已有协程在运行，则停止它
        ...
        // 启动新的协程
        currentPauseCoroutine = StartCoroutine(HandlePauseCardSelection());
        // 等待当前协程完成
        yield return currentPauseCoroutine;
    }

    private IEnumerator HandlePauseCardSelection()
    {
        // 禁用卡牌交互
        foreach (CardForAgent card in cards)
        {
            card.button.interactable = false;
        }

        // 等待冻结时间结束
        yield return new WaitForSeconds(cardInteractionPauseTime);
    ...
    public void ReGenerateCards()
    {
        ...
        foreach (var move in movesList.moves)
        {
            ...
            if (isSelectable)
            {
                ...
                CardForAgent cardComponent = card.GetComponent<CardForAgent>();
    ...
    // 检查卡牌选中状态
    public void CheckSelectCard(CardForAgent selectedCard)
    {
        // 遍历检查所有卡牌
        foreach (CardForAgent card in cards)
        {
    ...
    // 将卡牌居中
    public IEnumerator CenterCard(CardForAgent card)
    {
